...
(function () {
  ...
  var FAVORITES_KEY = 'v15-static-favorites';
  ...
  var THEME_STORAGE_KEY = 'v15-static-skin';
  ...
  function buildSkinPanelHtml() {
    ...
    /* /studio「公開皮膚」設定(v15-public-config.publicSkins)限縮前台可選清單 */
    var cfg = null;
    try { cfg = JSON.parse(localStorage.getItem('v15-public-config') || 'null'); } catch (e) {}
  ...
  function initSkinSwitchers() {
    $all('button[aria-label="Switch skin"]').forEach(function (btn) {
      ...
      function open() {
        ...
        updateSkinUI(document.documentElement.getAttribute('data-theme') || 'winpop');
  ...
  function initTheme() {
    ...
    applyTheme(saved || document.documentElement.getAttribute('data-theme') || 'winpop', false);
    initSkinSwitchers();
  }
   ...
   * 寫入 localStorage 的 v15-public-config;前台開機套用
   * + storage 事件跨分頁即時同步(換膚、站名、公開皮膚/語系範圍)。
   * ========================================================== */
  function publicConfig() {
    try { return JSON.parse(localStorage.getItem('v15-public-config') || 'null'); } catch (e) { return null; }
  }
  ...
  function initPublicConfigSync() {
    ...
    window.addEventListener('storage', function (ev) {
      if (ev.key === 'v15-public-config') applyPublicConfigNow();
    });
  }
  ...
  function applyStudioChromeNow() {
    ...
    var draft = null;
    try { draft = JSON.parse(localStorage.getItem('v15-studio-draft') || 'null'); } catch (e) { return; }
    if (draft && draft.chrome) applyChromeVariant(draft.chrome);
  }

  /* ============================================================
   * /studio 區塊草稿預覽(studio-draft.ts 對應):設計後台把首頁區塊順序/
   * 顯示開關寫進 localStorage['v15-studio-draft'];本函式依該草稿對本頁
  ...
  function initStudioSectionsSync() {
    ...
    applyStudioChromeNow();
    window.addEventListener('storage', function (ev) {
      if (ev.key === 'v15-studio-draft') { applyStudioSectionsNow(); applyStudioChromeNow(); }
    });
  }
  ...
  function buildAppHeaderMobileMenu() {
    ...
    return (
      ...
      '<img src="logo.svg" alt="Casino Logo" class="header-mobile-menu-logo">' +
  ...
  /* 圖1/圖2:Deposit/Withdrawal 從側欄清單拉出、獨立成頂部兩顆藥丸切換鈕
     (main.css .sidebar-mode-toggle);清單內原本的兩個 <a> 用 CSS 隱藏
     (main.css)避免重複,不從 DOM 移除以保留原本的 is-active/連結邏輯。
     文字用既有 I18N 原字串(Deposit/Withdrawal),交給下面 boot 階段的
     applyLocale() 一併套用當前語系,不用另外維護翻譯。 */
  function initSidebarModeToggle() {
    $all('.sidebar-nav-list').forEach(function (list) {
      var nav = list.parentElement;
      if (!nav || nav.querySelector('.sidebar-mode-toggle')) return;
      var current = pageName();
      var toggle = document.createElement('div');
      toggle.className = 'sidebar-mode-toggle';
      toggle.innerHTML =
        '<a href="deposit.html" class="sidebar-mode-btn' + (current === 'deposit' ? ' active' : '') + '">Deposit</a>' +
        '<a href="withdrawal.html" class="sidebar-mode-btn' + (current === 'withdrawal' ? ' active' : '') + '">Withdrawal</a>';
  ...
  function buildPromoDetailHtml(p) {
    return '' +
      ...
      '<img class="promotion-detail-logo" src="logo.svg" alt="WIN100%">' +
      '<div>' +
