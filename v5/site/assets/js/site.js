// CMS_前台_v5 — 純靜態站 vanilla JS 行為層（無框架、無 build）。
(function () {
  'use strict';
  ...
  /* studio（設計後台）套用首頁區塊顯示/站點名稱/skin：與 studio.js 共用同一把
     ...
     window.__cmsV5StudioApply 核心函式立即反映，不必等按下「套用到本站」、
     也不必整頁重整。 */
  var ORIGINAL_TITLE = document.title;
  var STUDIO_SECTIONS_KEY = 'cms-v5-studio-sections';
  var STUDIO_SITENAME_KEY = 'cms-v5-studio-sitename';
  var STUDIO_SKIN_KEY = 'cms-v5-studio-skin';
  var STUDIO_LAYOUT_KEY = 'cms-v5-studio-layout';
  ...
  function applySiteName(name) {
    document.title = name ? ORIGINAL_TITLE.replace(/^CMS_前台_v5/, name) : ORIGINAL_TITLE;
  }
  function applySkin(skinId) {
    document.documentElement.setAttribute('data-skin', skinId || 'ignite-classic');
  }
  ...
  /* 12 欄版位的「直接在預覽畫面拖曳排序」:只有 studio 呼叫過
     __cmsV5StudioSetEditMode(true) 才會啟用,一般訪客直接開 index.html
  ...
  function notifyStudioReorder() {
    try { window.parent.postMessage({ type: 'cms-v5-studio-reorder', layout: currentGridLayout() }, location.origin); } catch (e) {}
  }
  ...
  window.__cmsV5StudioSetEditMode = function (on) {
    gridEditModeOn = !!on;
    document.documentElement.classList.toggle('cms-v5-grid-edit', gridEditModeOn);
    if (gridEditModeOn) safe(initGrid12DragEdit);
  };

  /* studio 父頁（同源）在 iframe load 後或任何控制項變動時直接呼叫這個函式，
     即時把草稿反映到畫面上，不經過 localStorage、不需重整。 */
  window.__cmsV5StudioApply = function (draft) {
    if (!draft) return;
  ...
  on(document, 'cms-v5:locale-changed', function () {
    renderHeaderAuth();
    if (currentPage() === 'withdrawal.html') renderWithdrawalUI();
  });
  function doLogin(name) {
    saveAuth({ name: name || tr('auth.defaultMemberName', '會員'), balance: DEFAULT_BALANCE, points: DEFAULT_POINTS });
    renderHeaderAuth();
  }
  function doLogout() {
    saveAuth(null);
  ...
  var MEMBER_MENU_ITEMS = [
    { href: 'account.html', labelKey: 'sidebar.overview', label: '帳戶總覽', icon: '<rect width="7" height="9" x="3" y="3" rx="1"/><rect width="7" height="5" x="14" y="3" rx="1"/><rect width="7" height="9" x="14" y="12" rx="1"/><rect width="7" height="5" x="3" y="16" rx="1"/>' },
    { href: 'betting-record.html', labelKey: 'sidebar.bettingRecord', label: '投注紀錄', icon: '<path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/>' },
    { href: 'deposit-record.html', labelKey: 'sidebar.depositRecord', label: '儲值紀錄', icon: '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M16 13H8"/><path d="M16 17H8"/>' },
    { href: 'profit-loss.html', labelKey: 'sidebar.profitLoss', label: '損益報表', icon: '<path d="M16 7h6v6"/><path d="m22 7-8.5 8.5-5-5L2 17"/>' },
    { href: 'withdrawal-record.html', labelKey: 'sidebar.withdrawalRecord', label: '提款紀錄', icon: '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M16 13H8"/><path d="M16 17H8"/>' },
    { href: 'withdrawal-detail.html', labelKey: 'sidebar.withdrawalDetail', label: '提款明細', icon: '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M16 13H8"/><path d="M16 17H8"/>' },
    { href: 'account-record.html', labelKey: 'sidebar.accountRecord', label: '帳戶紀錄', icon: '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M16 13H8"/><path d="M16 17H8"/>' },
    { href: 'personal-info.html', labelKey: 'sidebar.personalInfo', label: '個人資料', icon: '<path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>' },
    { href: 'security.html', labelKey: 'sidebar.security', label: '安全中心', icon: '<path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/>' },
    { href: 'about.html?tab=faq', labelKey: 'notice.faq', label: '常見問題', icon: '<circle cx="12" cy="12" r="9"/><path d="M9.8 9a2.4 2.4 0 1 1 3.4 2.2c-.8.4-1.2.9-1.2 1.8"/><path d="M12 17h.01"/>' },
  ];
  ...
  function openMemberMenu() {
    ...
    var navHtml = '<div class="header-menu-section">' + tr('headerMenu.myAccount', '我的帳戶') + '</div>' +
      '<div class="header-menu-list">' + MEMBER_MENU_ITEMS.map(function (item) {
        var active = item.href.split('?')[0] === page;
        return '<a href="' + item.href + '" class="header-menu-list-item' + (active ? ' active' : '') + '"><span>' + escapeHtml(tr(item.labelKey, item.label)) + '</span><svg class="chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg></a>';
      }).join('') + '</div>';
    openMobileOverlay(navHtml);
  }
  ...
  function openAuthModal(mode) {
    ...
    wrap.innerHTML =
      ...
      '<button type="button" class="auth-modal-close" aria-label="' + tr('common.close', '關閉') + '" data-auth-close><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg></button></div>' +
  ...
  function openCsModal() {
    ...
    wrap.innerHTML =
      ...
      '<button type="button" class="cs-modal-close" aria-label="' + tr('common.close', '關閉') + '" data-cs-close><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg></button></div>' +
  ...
  function openChatWidget() {
    ...
    wrap.innerHTML =
      ...
      '<button type="button" class="chat-widget-close" data-chat-close aria-label="' + tr('common.close', '關閉') + '">' +
  ...
  function openPayModal(title, bodyHtml) {
    ...
    wrap.innerHTML =
      ...
      '<button type="button" class="auth-modal-close" aria-label="' + tr('common.close', '關閉') + '" data-pay-close><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg></button></div>' +
  ...
  function depositStepModal(methodId, amountVal) {
    ...
    if (isBank) {
      ...
      body =
        ...
        '<div style="text-align:center;margin-bottom:14px"><svg width="176" height="176" viewBox="0 0 29 29" shape-rendering="crispEdges" role="img" aria-label="' + tr('dp.paymentQrAria', '付款 QR Code') + '"><rect width="29" height="29" fill="#fff"></rect>' + fakeQrModules() + '</svg></div>' +
  ...
  /* 提款帳戶管理:localStorage 模擬已綁定的收款帳戶清單。分成「銀行帳戶」
     ...
     上限。 */
  var WD_ACCOUNTS_KEY = 'cms-v5-withdraw-accounts';
  /* 銀行帳戶上限 5 筆,加密錢包只能綁 1 筆(對照 v2 WALLET_ACCOUNTS 的
     (0/1) 上限,跟銀行帳戶的 (0/5) 不是同一個數字) */
  var WD_ACCOUNT_CAP = { bank: 5, crypto: 1 };
  // bank 沒有固定廠牌名,無 bankName 時要即時取當前語系的「銀行卡」,不能跟
  // trc20/erc20(協定名稱,各語系皆同)一樣寫死在物件裡,否則語系切換後不會更新。
  var WD_TYPE_LABEL = { trc20: 'USDT-TRC20', erc20: 'USDT-ERC20' };
  function wdTypeLabel(type) { return type === 'bank' ? tr('pay.bankCard', '銀行卡') : WD_TYPE_LABEL[type]; }
  var WD_TYPE_ICON = {
  ...
  function wdAccountBadgeText(acc) {
    return acc.type === 'bank' ? (acc.bankName || wdTypeLabel(acc.type)) : wdTypeLabel(acc.type);
  }
