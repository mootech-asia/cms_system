// CMS_Frontend_v5 — 純靜態站 vanilla JS 行為層（無框架、無 build）。
(function () {
  'use strict';
  ...
  function applySiteName(name) {
    document.title = name ? ORIGINAL_TITLE.replace(/^CMS_Frontend_v5/, name) : ORIGINAL_TITLE;
  }
  ...
  function initPromoPopup() {
    ...
    function todayKey() {
      ...
      return 'cms-v5-promo-popup-dismissed_' + d.getFullYear() + '-' + pad(d.getMonth() + 1) + '-' + pad(d.getDate());
    }
    ...
    function cardHTML(promo) {
      return (
        ...
        '<input type="checkbox" data-promo-popup-remember class="h-[18px] w-[18px] accent-accent">' +
