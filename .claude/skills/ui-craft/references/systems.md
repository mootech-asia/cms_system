<!-- 原文照搬自 https://github.com/s0xDk/refactoring-ui-skill (MIT license)，
     未翻譯。這份是「從零建立色板」的方法論——本 repo 各版本品牌色相已經定案，
     只在「新增一組 skin 換膚色板」時才會用到，且新色板要沿用該版本既有色相
     延伸，不是套用這裡的數值重新發明一套配色。 -->
