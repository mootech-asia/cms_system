...
`cms_system_v4` 四個交付版型，依版本號排序。除 v1.5 外，每個版本都是完全獨立、免建置的
純 HTML+CSS+JS 靜態站，不依賴任何前端框架（無 Vue／Nuxt／Tailwind／PrimeVue 殘留）。

## 目錄結構

```
cms_system/
├── index.html       # 統一首頁，各版型入口（依版本號排序：v1.5 → v2 → v3 → v4）
├── v1.5/
